...
export const registerModifier = (name?: string) => (modifier: new () => CDOTA_Modifier_Lua) => {
    ...
    print('link: ', name, fileName, type);
    LinkLuaModifier(name, fileName, type);
};

/**
 * Calls a spawn function on script entities
 * @returns
 */
export const registerUnitScript = () => (customNPCScript: (new () => UnitScript) ) => {
    const [env] = getFileScope();

    const clazz = new customNPCScript();
    env[clazz.constructor.name] = clazz;
    env['Spawn'] = function (this: void, arg: CScriptKeyValues) {
        clazz.Spawn(env['thisEntity']);
    };
};
