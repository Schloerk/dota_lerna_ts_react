 ...
 * @param {(data: XNetTableDefinations[T][K]) => void} callback
 */
export function onXNetTableKeyEvent<T extends keyof XNetTableDefinations, K extends keyof XNetTableDefinations[T]>(
    table_name: T,
    key: K,
    callback: (data: XNetTableDefinations[T][K]) => void
) {
    return onLocalEvent(`x_net_table`, data => {
        if (data.table_name.toString() === table_name && data.key.toString() === key) {
            callback(data.content);
        }
    });
...
type a<T extends keyof XNetTableDefinations> = XNetTableDefinations[T];

export function onXNetTableEvent<T extends keyof XNetTableDefinations, S extends keyof XNetTableDefinations[T]>(
    table_name: T,
    callback: (key: string, data: XNetTableDefinations[T][S]) => void
) {
    return onLocalEvent(`x_net_table`, data => {
        if (data.table_name.toString() === table_name) {
            callback(data.key, data.content);
        }
    });
}
